...
public:
    ...
    objLoader(objLoader &obj,const char* addr) {
        ...
        if (!file.is_open()) {
            std::cerr << "\033[31mfile open error\033[m" << std::endl;
...
private:
    ...
    void readFace(std::istringstream& iss, std::vector<GLuint>& vInd, std::vector<GLuint>& vtInd, std::vector<GLuint>& vnInd) {
        std::string tokens;
        while (iss >> tokens) {
