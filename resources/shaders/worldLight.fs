...
void main() {
    ...
    // 计算最终的镜面反射光强度
    vec3 specular = light.specular * material.specular * pow(cosHN, material.shininess*1.5)*texture(fTex, fTexCoord).rgb;
    ...
    // 计算光源到顶点的距离
    float distance = length(viewLightPos - viewVerPos);
